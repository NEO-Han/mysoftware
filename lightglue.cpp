...
LightGlue::LightGlue(std::wstring modelPath, Stitcher::Mode mode, float matchThresh)
{
	...
	m_env= Ort::Env(ORT_LOGGING_LEVEL_WARNING, "LightGlue");

	m_sessionOptions.SetIntraOpNumThreads(2);
	m_sessionOptions.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_ALL);
	m_matcherSession = new Ort::Session(m_env, modelPath.c_str(), m_sessionOptions);
...
LightGlue::~LightGlue()
{
	...
	if (m_matcherSession != nullptr)
	{
		delete m_matcherSession;
		m_matcherSession = nullptr;
	}
}
	...
	MatchesInfo& matches_info)
{
	...
	//static Ort::Session lightglueSession(m_env, this->m_modelPath.c_str(), m_sessionOptions);
	...
	vector<Ort::Value> outputs = m_matcherSession->Run(this->run_options, input_names, inputTensor.data(), 4, output_names, 4);
